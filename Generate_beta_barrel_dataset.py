...
# Filters the domain descriptions list for beta-barrels, picking out PDB
...
with open('Unprocessed_beta_barrel_pdbs.txt', 'w') as unprocessed_file:
...
domain_dict.to_csv('CATH_2.40_domain_desc_DataFrame_v_4_1_0.csv')
domain_dict.to_pickle('CATH_2.40_domain_desc_DataFrame_v_4_1_0.pkl')
...
# Generates a list of the domain descriptions provided in
# CATH_domain_description_v_4_1_0.txt
if not os.path.isdir('Beta_barrel_PDB_files'):
    os.mkdir('Beta_barrel_PDB_files')
...
domain_dict = pd.DataFrame({'PDB_CODE': domain_pdb_ids,
                            ...
                            'SSEQS': domain_sseqs,
                            'SSEQS_START_STOP': domain_sseqs_start_stop})
domain_dict.to_csv('Beta_barrel_PDB_files/CATH_2.40_domain_desc_DataFrame_v_4_1_0.csv')

# Downloads a copy of each PDB file listed in the CATH database as having a
# beta-barrel architecture
if os.path.isdir('Beta_barrel_PDB_files/RCSB_PDB_files'):
    shutil.rmtree('Beta_barrel_PDB_files/RCSB_PDB_files')
os.mkdir('Beta_barrel_PDB_files/RCSB_PDB_files')
for pdb_id in set(domain_pdb_ids):
    print('Downloading {} from the RCSB PDB website'.format(pdb_id))
    url = 'http://www.rcsb.org/pdb/files/{}.pdb'.format(pdb_id.upper())
    with open('Beta_barrel_PDB_files/RCSB_PDB_files/{}.pdb'.format(pdb_id), 'w') as pdb_file:
        pdb_file.write(requests.get(url).text)

if not os.path.isdir('Beta_barrel_PDB_files/Segment_PDB_files'):
    os.mkdir('Beta_barrel_PDB_files/Segment_PDB_files/')

# Generates a pdb file listing the coordinates of each segment sequence (SSEQS)
unprocessed_list = []
for row in range(0, len(domain_pdb_ids)):
    print(domain_dict['PDB_CODE'][row])
    ...
    for index_1, segment in enumerate(domain_dict['SSEQS'][row]):
        ...
        pdb_file_lines = []
        with open('Beta_barrel_PDB_files/RCSB_PDB_files/{}.pdb'.format(domain_dict['PDB_CODE'][row])) as pdb_file:
            for line in pdb_file:
                whitespace_remv_line = line.replace(' ', '')
                if whitespace_remv_line.startswith('EXPDTATHEORETICAL'):
                    break
                elif line[0:6].strip() in ['ATOM', 'HETATM', 'TER']:
        ...
        for index_3, sequence in enumerate(sequences):
            ...
            if similarity > 0.95:
                sequence_identified = True
                with open('Beta_barrel_PDB_files/Segment_PDB_files/{}_{}_{}.pdb'.format(
                    domain_dict['CATHCODE'][row], domain_dict['PDB_CODE'][row],
                    domain_dict['SEGMENT_ID'][row][index_1]
                    ), 'w') as pdb_file:
                        for index_4 in indices[index_3]:
                            pdb_file.write(pdb_file_lines[index_4])

            if sequence_identified is True:
                break

        if similarity <= 0.95:
            unprocessed_list.append(domain_dict['PDB_CODE'][row])

with open('Beta_barrel_PDB_files/Unprocessed_beta_barrel_pdbs.txt', 'w') as unprocessed_file:
    for pdb_file in unprocessed_list:
        unprocessed_file.write(pdb_file)
"""
