...
class naccess_solv_acsblty_calcs():
    ...
    def calculate_int_ext_barrel(domain_id, dssp_df, sheets,
        ...
        sheet_nums = [sheet.replace('{}_sheet_'.format(domain_id), '') for
                      sheet in sheets]
        sheets_df = dssp_df[dssp_df['SHEET_NUM'].isin(sheet_nums)]
        ...
        for strand in strands:
            ...
            if face_1_solv_acsblty > face_2_solv_acsblty:
                ext_chain_res_num = strand_res[::2]
                int_chain_res_num = strand_res[1::2]
            elif face_1_solv_acsblty < face_2_solv_acsblty:
                ext_chain_res_num = strand_res[1::2]
                int_chain_res_num = strand_res[::2]
            else:
                ...
                sec_struct_dfs_dict[domain_id] = None
                unprocessed_list.append(domain_id)
                return sec_struct_dfs_dict, unprocessed_list
    ...
    def calculate_int_ext_sandwich(domain_id, dssp_df, sheets,
        ...
        # Determines solvent accessiblity of each residue in the beta-sheet
        ...
        for res in list(int_ext_combined.keys()):
            solv_ascblty_combined = int_ext_combined[res]
            solv_ascblty_indv = int_ext_indv[res]

            if solv_ascblty_combined < solv_ascblty_indv:
                int_ext_combined[res] = 'interior'
            elif solv_ascblty_combined == solv_ascblty_indv:
                int_ext_combined[res] = 'exterior'
            else:
                print('ERROR: solvent accessiblity in sheet complex is '
...
class calculate_solvent_accessible_surface_area(run_stages):
    ...
    def run_naccess(self, sec_struct_dfs_dict, domain_sheets_dict):
        ...
        if self.code[0:4] in ['2.40']:
            if len(list(sec_struct_dfs_dict.keys())) != len(list(domain_sheets_dict.keys())):
                print('ERROR: Incorrect number of sheets retained')
                unprocessed_list.append(domain_id)
        elif self.code[0:4] in ['2.60']:
            if 2(len(list(sec_struct_dfs_dict.keys()))) != len(list(domain_sheets_dict.keys())):
                print('ERROR: Incorrect number of sheets retained')
                unprocessed_list.append(domain_id)
