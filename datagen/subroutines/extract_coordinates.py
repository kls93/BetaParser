...
class extract_beta_structure_coords(run_stages):
    ...
    def gen_cdhit_dict(self, filtered_domain_df):
        # Loads the list of FASTA sequences generated by the CD-HIT web server
        fasta_list = []
        with open(self.cdhit_output, 'r') as chain_entries_file:
            cdhit_lines = chain_entries_file.read()
            cdhit_lines = cdhit_lines.split('>')
            for seq in cdhit_lines:
                seq = seq.split('\n')[1:]
                seq = ''.join(seq)
                fasta_list.append(seq)
        fasta_list = [seq for seq in fasta_list if seq != '']
